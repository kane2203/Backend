...
const corsOptions = {
  origin: ["https://budgettrip.onrender.com"],
...
app.use((req, res, next) => {
  // req.store = store;
  res.setHeader("Access-Control-Allow-Origin", "https://budgettrip.onrender.com");
...
app.get("/collections", async (req, res) => {
  try {
    ...
    const collections = await client.db("Budgetrip").listCollections().toArray();
...
app.delete("/deleteRecord/:collection/:id", async (req, res) => {
  ...
  try {
    ...
    console.error(`Error deleting record with ID ${dataId} from collection ${collectionName}:`, error);
    res.status(500).json({
      error: "Internal Server Error"
    });
  }
