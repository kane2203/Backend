...
exports.forgotPassword1 = catchAsyncErrors(async (req, res, next) => {
  ...
  const resetPasswordUrl = `${req.protocol}://${req.get("host")}/api/v1/password/resetv/${resetToken1}`;
...
exports.resetPassword1 = catchAsyncErrors(async (req, res, next) => {
  // creating token hash
  const resetPasswordToken1 = crypto.createHash("sha256").update(req.params.token1).digest("hex");
  const vendor = await Vendor.findOne({
    resetPasswordToken1,
    resetPasswordExpire: {
      $gt: Date.now()
...
exports.getSingleVendor = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!vendor) {
    return next(new ErrorHander(`Vendor does not exist with Id: ${req.params.id}`));
  }
  res.status(200).json({
    success: true,
    vendor
...
exports.deleteVendor = catchAsyncErrors(async (req, res, next) => {
  const vendor = await Vendor.findById(req.params.id);
  if (!vendor) {
    return next(new ErrorHander(`Vendor does not exist with Id: ${req.params.id}`, 400));
  }
