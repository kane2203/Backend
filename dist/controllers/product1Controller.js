...
exports.getAllProducts1 = catchAsyncErrors(async (req, res, next) => {
  ...
  const apiFeature = new ApiFeatures(Product1.find(), req.query).search1().filter();
...
exports.getvendorProducts1 = catchAsyncErrors(async (req, res, next) => {
  ...
  const vproducts1 = await Product1.find({
    vendor: id
  });
...
exports.createProduct1Review = catchAsyncErrors(async (req, res, next) => {
  ...
  const isReviewed = product1.reviews.find(rev => rev.user.toString() === userId);
  if (isReviewed) {
...
exports.deleteReview1 = catchAsyncErrors(async (req, res, next) => {
  ...
  const reviews = product1.reviews.filter(rev => rev._id.toString() !== req.query.id.toString());
  let avg = 0;
  reviews.forEach(rev => {
    avg += rev.rating;
  });
  ...
  await Product1.findByIdAndUpdate(req.query.product1Id, {
    reviews,
    ratings,
    numOfReviews
  }, {
