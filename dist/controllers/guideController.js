...
exports.registerGuide = catchAsyncErrors(async (req, res, next) => {
  ...
  if (password.length < 8 || !/[!@#$%^&*]/.test(password) || !/\d/.test(password) || !/[A-Z]/.test(password)) {
    return res.status(400).json({
      error: "Invalid password. It must contain at least 8 characters, one special character, one number, and one capital letter."
    });
  }
...
exports.forgotPassword2 = catchAsyncErrors(async (req, res, next) => {
  ...
  const resetPasswordUrl = `${req.protocol}://${req.get("host")}/api/v1/password/resetg/${resetToken2}`;
...
exports.resetPassword2 = catchAsyncErrors(async (req, res, next) => {
  // creating token hash
  const resetPasswordToken2 = crypto.createHash("sha256").update(req.params.token2).digest("hex");
  const guide = await Guide.findOne({
    resetPasswordToken2,
    resetPasswordExpire: {
      $gt: Date.now()
...
exports.updateProfile2 = catchAsyncErrors(async (req, res, next) => {
  ...
  const guide = await Guide.findByIdAndUpdate(req.params.guideId, newGuideData, {
    new: true,
    runValidators: true,
...
exports.getSingleGuide = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!guide) {
    return next(new ErrorHander(`Guide does not exist with Id: ${req.params.id}`));
  }
  res.status(200).json({
    success: true,
    guide
...
exports.deleteGuide = catchAsyncErrors(async (req, res, next) => {
  const guide = await Guide.findById(req.params.id);
  if (!guide) {
    return next(new ErrorHander(`Guide does not exist with Id: ${req.params.id}`, 400));
  }
