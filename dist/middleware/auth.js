...
exports.authorizeRoles1 = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.vendor.role)) {
      return next(new ErrorHander(`Role: ${req.vendor.role} is not allowed to access this resouce `, 403));
    }
    next();
  };
};
exports.authorizeRoles2 = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.guide.role)) {
      return next(new ErrorHander(`Role: ${req.guide.role} is not allowed to access this resouce `, 403));
    }
    next();
  };
};